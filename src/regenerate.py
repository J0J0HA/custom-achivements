...
def regenerate(urls):
    ...
    def regenerate_from_url(url):
        try:
            json = requests.get(url, timeout=30, headers={'Cache-Control': 'no-cache'}).json()
        ...
        for achievement in achievements:
            ...
            for level, level_dat in enumerate(achievement.get("levels", []), start=1):
                ...
                trigger = rutils.new_trigger(trigger_dat.get("name", "achieve.manual_assign"), trigger_dat.get("value", -1))
                rutils.new_achievement(row, level, level_dat.get("description", "No description provided."), trigger)
